import React, { Component } from "react";

const HowToChoose = () => {
    return (
      <>
        <section className="why-choose-us">
          <div className="why-chooes-us-service-work">
            <div className="container">
              <div className="row">
                <div className="col-sm-12 col-md-12 col-lg-6 col-xl-6 desk-img"></div>
                <div className="col-sm-12 col-md-12 col-lg-6 col-xl-6">
                  <div className="choose-content">
                    <div className="section-title">
                      <h2>How To Choose Better One For Your Place</h2>
                    </div>
                    <div className="section-bottom">
                      <h3>Why Choose Us</h3>
                    </div>
                    <p>
                      Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                      Integer nec odio. Praesent libero. Sed cursus ante dapibus
                      diam. Sed nisi. Nulla quis sem at nibh elementum
                      imperdiet. Duis sagittis ipsum. Praesent mauris. Fusce nec
                      tellus sed augue semper porta. Mauris massa. Vestibulum
                      lacinia arcu eget nulla. className aptent taciti sociosqu
                      ad litora torquent per conubia nostra, per inceptos
                      himenaeos. Curabitur sodales ligula in libero. Sed
                      dignissim lacinia nunc.
                    </p>
                    <p>
                      Curabitur tortor. Pellentesque nibh. Aenean quam. In
                      scelerisque sem at dolor. Maecenas mattis. Sed convallis
                      tristique sem. Proin ut ligula vel nunc egestas porttitor.
                      Morbi lectus risus, iaculis vel, suscipit quis, luctus
                      non, massa. Fusce ac turpis quis ligula lacinia aliquet.
                      Mauris ipsum. Nulla metus metus, ullamcorper vel,
                      tincidunt sed, euismod in, nibh. Quisque volutpat
                      condimentum velit. className aptent taciti sociosqu ad
                      litora torquent per conubia nostra, per inceptos
                      himenaeos.
                    </p>
                    <ul>
                      <li>Free, Friendly Service</li>
                      <li>Educational Approach</li>
                      <li>Engaging, Online Platform</li>
                      <li>Multiple Offers to Choose</li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
      </>
    );
}
export default HowToChoose;